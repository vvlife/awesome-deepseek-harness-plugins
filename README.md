...
- [satan9394/dsh-self-improving-agent](https://github.com/satan9394/dsh-self-improving-agent) (★0) — Self-improving AI agent: built-in closed learning loop (auto-create skills from experience, improve during use, FTS5 session search + LLM summaries for cross-session recall, Honcho dialectic user modeling), cross-platform gateway (Telegram/Discord/Slack/WhatsApp), cron natural-language scheduling, subagent parallelism, serverless hibernate/wake.
- [satan9394/dsh-social-simulation](https://github.com/satan9394/dsh-social-simulation) (★0) — Million-agent social simulation: scalable open-source social-media simulator where LLM agents mimic up to 1M users on Twitter/Reddit-style platforms, 23 actions, interest/hot-score recommendation, dynamic environments — study information spread, group polarization, herd behavior.
- [satan9394/dsh-autonomous-research](https://github.com/satan9394/dsh-autonomous-research) (★0) — Autonomous LLM training research: give an agent a small real training setup to experiment overnight (edit code → fixed 5-min train → check val_bpb → keep/discard loop, ~12 experiments/hour), single-file modification, program.md as ultra-lightweight skill.
- [satan9394/dsh-cloud-well-architected](https://github.com/satan9394/dsh-cloud-well-architected) (★0) — Cloud Well-Architected six-pillar review: reliability (user-experience goals, SLO + error budgets, cross-region redundancy, horizontal scaling, golden signals, graceful degradation, recovery tests, blameless postmortems), cost, operational excellence, performance, security, sustainability.
- [satan9394/dsh-skill-optimization](https://github.com/satan9394/dsh-skill-optimization) (★0) — Train agent skills like neural networks: epochs/batchsize/learning-rate/validation gates without touching model weights, rollout→reflect→aggregate→select→update→evaluate loop, candidate edits accepted only on strict held-out improvement, zero inference-time overhead, deploy compact best_skill.md.
- [satan9394/dsh-geo-seo](https://github.com/satan9394/dsh-geo-seo) (★0) — GEO-first AI search optimization: optimize sites for AI engines (ChatGPT/Claude/Perplexity/Gemini/AI Overviews), citability scoring, AI crawler analysis, llms.txt generation, brand-mention scanning, Schema + E-E-A-T, 5 parallel subagent full audit.
- [satan9394/dsh-virtual-eng-team](https://github.com/satan9394/dsh-virtual-eng-team) (★0) — Virtual engineering team: 23 specialist roles (CEO rethinks product, eng manager locks architecture, designer catches AI slop, reviewer finds prod bugs, QA opens a real browser, security officer runs OWASP+STRIDE, release engineer ships PRs), slash-command flow /office-hours → /autoplan → /review → /qa → /ship.
- [satan9394/dsh-knowledge-brain](https://github.com/satan9394/dsh-knowledge-brain) (★0) — Knowledge brain layer: search gives pages, brain gives cited answers — synthesis layer + self-wiring knowledge graph (zero-LLM entity edges) + gap analysis (what the brain doesn't know) + 24/7 dream cycle consolidation + company-brain per-login isolation.
- [satan9394/dsh-channel-assistant](https://github.com/satan9394/dsh-channel-assistant) (★0) — Channel-first personal assistant: one Gateway connects models/tools/messaging channels/companion apps, channels-as-interface (WhatsApp/Telegram/Slack/Discord/Signal/iMessage), Control UI/CLI/TUI, pairing-based security for unknown senders, sandboxed tool execution, ClawHub plugin ecosystem.
- [satan9394/dsh-desktop-agent-gui](https://github.com/satan9394/dsh-desktop-agent-gui) (★0) — Desktop GUI for agent orchestration: zero-config wizard, intelligent chat (@agent routing, inline skill cards, doc previews), multi-channel multi-account management, cron automation delivering to external channels, local-first skill management, keychain-secured provider credentials.
- [satan9394/dsh-model-gateway](https://github.com/satan9394/dsh-model-gateway) (★0) — Model routing gateway: one endpoint to 290+ AI providers (90+ free, 1200+ models), Claude Code/Codex/Cursor/Cline/Copilot compatible, quota-aware auto-fallback, 19 routing strategies, RTK+Caveman stacked compression saving 15-95% tokens, honest free-tier budget (~1.5B tokens/mo).
- [satan9394/dsh-parallel-agent-ade](https://github.com/satan9394/dsh-parallel-agent-ade) (★0) — Parallel agent development environment: Codex/ClaudeCode/OpenCode/Pi side-by-side each in its own git worktree, fan one prompt across agents and merge the winner, mobile companion monitoring, Design Mode (click UI element → HTML/CSS/screenshot into prompt), SSH remote worktrees, annotate-AI-diff loop.
- [satan9394/dsh-doc-compiled-skills](https://github.com/satan9394/dsh-doc-compiled-skills) (★0) — Precompile official docs into executable skills: extract task-oriented capabilities → classify (limits/troubleshooting/config/architecture/security) → reveal on user intent; knowledge pre-compiled into actions/choices/guardrails instead of per-query RAG retrieval.
- [satan9394/dsh-html-template-library](https://github.com/satan9394/dsh-html-template-library) (★0) — Reusable HTML template library methodology: 34 design systems (Soft Editorial / Neo-Grid Bold / Stencil & Tablet / Sakura Chroma / Vellum / Monochrome…), agent reads index.json, matches the brief, clones and adapts — 3 multi-layout previews + metadata + reusable design tokens per template.
- [satan9394/dsh-taste-review](https://github.com/satan9394/dsh-taste-review) (★0) — Taste-driven judgment call for ambiguous decisions: state the question plainly, list files to consider, weigh a few options, give recommendation with alternatives; depth matches the decision weight — design calls may warrant several paragraphs, naming calls may not.
- [satan9394/dsh-test-desktop-app](https://github.com/satan9394/dsh-test-desktop-app) (★0) — Electron desktop app E2E testing via CDP: connect to real Chromium renderer for DOM inspection, JS evaluation, clicks, and screenshots; DPR×2 multiplier, real Input.dispatchMouseEvent for right-clicks, stacking checks for popup screenshots, debounced xterm refit.
...
- [nickhelion/dsh-qwen-token-plan-cn-responses](https://github.com/nickhelion/dsh-qwen-token-plan-cn-responses) (★0) — Qwen Token Plan CN Personal Responses API provider: official model/tool catalog sync, server-side web/code/extraction/image tools, DSH local functions, and image input.
